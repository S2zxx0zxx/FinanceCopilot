...
export function ResourceState({loading,error,retry}: {loading:boolean;error:string|null;retry:()=>void}) {
  if (loading) return <div role="status" className="premium-card p-10 flex items-center justify-center gap-3"><Loader2 aria-hidden="true" className="animate-spin h-5 w-5" /> Loading your data…</div>;
