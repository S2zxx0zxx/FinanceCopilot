...
export function LoadingState({ type = "page" }: { type?: "page" | "card" | "list" | "chart" }) {
  if (type === "card") {
    return <SkeletonCard className="h-[120px]" />;
  }
  if (type === "list") {
    return (
  ...
  if (type === "chart") {
    return (
      <div className="premium-card p-5">
        ...
        <div className="h-[200px] skeleton rounded-[12px]" />
      </div>
    );
  }
  // page
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      <div>
        <SkeletonText width="40%" className="h-[28px]" />
        <div className="mt-2"><SkeletonText width="30%" /></div>
      </div>
      <SkeletonCard className="h-[200px]" />
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[1, 2, 3, 4].map((i) => <SkeletonCard key={i} className="h-[100px]" />)}
      </div>
...
export function ApiErrorState({
  ...
  return (
    <div className="premium-card p-8 flex flex-col items-center text-center gap-3">
      <div className="w-12 h-12 rounded-full bg-[var(--negative-light)] flex items-center justify-center">
        <svg className="w-6 h-6 text-[var(--negative)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
      </div>
      <h3 className="font-display font-semibold text-[16px]">Couldn't load data</h3>
      <p className="text-[14px] text-[var(--text-secondary)] max-w-sm">{message}</p>
      {onRetry && (
        <button
          onClick={onRetry}
          className="mt-2 px-4 py-2 rounded-[10px] bg-[var(--accent)] text-white text-[13px] font-semibold hover:opacity-90 transition-opacity"
        >
          Retry
        </button>
      )}
