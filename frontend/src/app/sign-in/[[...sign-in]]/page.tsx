...
// sign-in CTA that bounces authenticated users back to the dashboard. In
// production, swap this for Clerk's <SignIn /> component once ClerkProvider is
// wired up in providers.tsx.
export default function SignInPage() {
  const router = useRouter();
  const { toast } = useToast();

  const handleContinue = () => {
    toast({ title: "Welcome back", description: "You're signed in to the preview." });
    router.push("/");
  };
      ...
      >
        <div className="premium-card p-8 flex flex-col gap-6">
          <div className="flex flex-col items-center text-center gap-3">
            ...
            <div>
              <h1 className="font-display font-bold text-[24px] tracking-[-0.02em]">Sign in to FinCopilot</h1>
              <p className="text-[13px] text-(--text-secondary) mt-1">
                Your AI co-pilot for money. Track spending, plan goals, forecast cash flow.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-[12px] text-(--text-secondary) pt-3 border-t border-[var(--border-subtle)]">
            ...
            <Link href="/" className="text-(--text-tertiary) hover:text-foreground flex items-center gap-1">
