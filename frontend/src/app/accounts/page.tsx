          ...
          return (
            <motion.div key={acc.account_id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.05 }}>
              <Link href={`/accounts/${acc.account_id}`} className="premium-card p-5 flex items-center gap-4 group">
                ...
                <div className="text-right"><p className={`text-[18px] font-display font-semibold tabular-nums ${isCredit ? "text-(--negative)" : ""}`}>{formatPaise(acc.balances.available_balance_paise)}</p><FreshnessBadge status={acc.last_synced_at.startsWith("2026-09-01") ? "live" : "recent"} /></div>
              </Link>
            </motion.div>
          );
        })}
