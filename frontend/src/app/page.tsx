...
import { FileUp, Flame, Gauge, Sparkles, Target } from "lucide-react";
...
import {
  ...
  IncomeMetricCard,
  SafeToSpendCard,
} from "@/components/dashboard/dashboard-premium";
import dashboardStyles from "@/components/dashboard/dashboard-premium.module.css";
...
const dashboardSources = {
  ...
  transactions: () => api.getTransactions({ limit: "5" }),
...
const quickActions = [
  { href: "/transactions", title: "Import a statement", description: "Bring your latest activity in", icon: FileUp, tone: "import" as const },
  { href: "/budgets", title: "Set a spending limit", description: "Give each category a plan", icon: Gauge, tone: "budget" as const },
  { href: "/goals", title: "Build a milestone", description: "Save towards something meaningful", icon: Target, tone: "goal" as const },
  { href: "/ai/chat", title: "Ask your copilot", description: "Explore your financial questions", icon: Sparkles, tone: "copilot" as const },
];
...
function MoneyValue({ value, className }: { value: unknown; className?: string }) {
  ...
  return parsed === null
    ? <span className={className}>Unavailable</span>
    : <CountUp value={parsed} format={v => formatPaise(Math.round(v))} className={className} />;
}
...
export default function HomePage() {
  ...
  const home: Record<string, unknown> = {
    ...object(money.net_position ?? {}),
    ...object(rawHome.safe_to_spend ?? {}),
  ...
  // Current backend contract returns `insights`; retain `feed` as a backwards-safe fallback.
  const insights = rows(data.insights.insights || data.insights.feed || []);
  const spending = { categories: rows(rawSpending.categories || []) };
  const incomeSources = rows(income.sources || []);
  const user = Object.keys(object(rawUser.user ?? {})).length ? object(rawUser.user ?? {}) : rawUser;
  const activeAccounts = amount(object(money.coverage ?? {}).total_accounts);
  const sts = amount(home.safe_to_spend_paise);
  const stsHorizonDays = amount(home.horizon_days);
  const stsPlanningDataStatus = label(home.planning_data_status, "unknown");
    ...
    <div className="flex flex-col gap-5 max-w-5xl">
      ...
      >
        <div className="min-w-0">
          <h1 className="font-display font-bold text-[28px] tracking-[-0.025em] text-(--text)">Dashboard</h1>
      ...
      {failures.length > 0 && (
        <div role="status" className="premium-card p-4 flex flex-wrap items-center justify-between gap-3 text-sm">
          <p>Some panels could not load: {failures.join(", ")}. Available records remain visible.</p>
          <button onClick={state.reload} className="min-h-11 px-4 rounded-xl bg-(--surface-subtle) text-accent">Retry panels</button>
        </div>
      )}

      <section aria-label="Dashboard quick actions" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {quickActions.map(action => <DashboardQuickAction key={action.href} {...action} />)}
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
        <motion.section
          ...
          className={`lg:col-span-2 ${dashboardStyles.moneyShell}`}
          aria-label="Money snapshot"
        >
          <DashboardMoneyCard amountPaise={amount(home.available_balance_paise)} activeAccounts={activeAccounts} />
          <div className={dashboardStyles.moneyActions}>
            <Link href="/data-coverage" className={dashboardStyles.moneyActionLink}>Review data coverage</Link>
            <Link href="/transactions" className={dashboardStyles.moneyActionLink}>Import activity</Link>
            <Link href="/accounts" className={dashboardStyles.moneyActionLink}>Explore accounts</Link>
          </div>
        </motion.section>
        ...
        <motion.div
          ...
          className="bg-(--surface) border border-border rounded-[20px] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.04)] flex flex-col"
        >
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[15px] font-semibold text-(--text)">Spending</h3>
            <span className="text-[11px] text-(--text-muted) font-mono">This Month</span>
          </div>
          ...
          <div className="mt-4 flex items-end gap-2 h-16" aria-label="Amounts of debit entries within your five most recent transactions">
            ...
            {recentDebits.map((tx, i) => (
              <div
                key={label(tx.transaction_id, String(i))}
                title={`${label(tx.merchant_name)}: ${formatPaise(amount(tx.amount_paise) ?? 0)}`}
                className="flex-1 rounded-t-lg bg-linear-to-t from-accent to-(--accent-bright)"
                style={{ height: `${Math.max(2, ((amount(tx.amount_paise) ?? 0) / peak) * 100)}%` }}
              />
            ))}
          </div>
          <p className="text-[10px] text-(--text-tertiary) mt-2">Debit entries in your latest five transactions</p>
        </motion.div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
          <SafeToSpendCard value={sts} horizonDays={stsHorizonDays} planningDataStatus={stsPlanningDataStatus} />
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.35 }}>
          <IncomeMetricCard value={amount(home.this_month_income_paise)} sourceCount={incomeSources.length} />
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-linear-to-br from-(--accent-dim) to-(--gold-light) border border-border rounded-[20px] p-5"
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-[11px] font-mono uppercase tracking-[0.08em] text-(--text-muted)">Your Streak</span>
            <div className="w-8 h-8 rounded-[10px] bg-linear-to-br from-accent to-(--gold) flex items-center justify-center">
