...
BROWSER_ORIGIN = "https://fincopilot.test"
...
async def test_register_and_list_passkey(
    ...
    assert options_data["options"]["rp"]["name"] == "FinCopilot"
...
async def test_register_options_reject_insecure_origin(client: AsyncClient, auth_headers: dict):
    response = await _register_options(client, auth_headers, "http://fincopilot.example.com")
...
async def test_register_options_use_configured_rp_id_for_subdomain(
    ...
    settings = SimpleNamespace(webauthn_rp_id="example.com", webauthn_origin="", webauthn_rp_name="FinCopilot")
