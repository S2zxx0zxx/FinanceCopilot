...
export function ChatDemo({
  ...
  const { activeIndex = 0, phase = "question", next, setPhase } = useChatCycle();
  const example = chatExamples[activeIndex] || chatExamples[0];
              ...
              <div className="max-w-[85%] rounded-[14px] rounded-tl-[4px] bg-[var(--surface-2)] border border-[var(--border)] px-3 py-2.5 text-[13px] flex flex-col gap-2">
                ...
                {phase === "answer" && (
                  <>
                    ...
                    {/* Rich response card */}
                    {example?.a && typedAnswer.length >= example.a.length - 2 && example.card && (
